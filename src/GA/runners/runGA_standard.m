function results = runGA_standard(config)
% Run standard GA with given configuration
% Returns results structure

    % Start diary
    timestamp = datestr(now, 'yyyy-mm-dd_HH-MM-SS');
    diaryFile = sprintf('output/GA_execution_%s.txt', timestamp);
    diary(diaryFile);
    diary on;
    
    fprintf('=== GA EXECUTION ===\n');
    ...
    for run = 1:config.execution.numRuns
        ...
        fprintf('Run %d completed in %.2f seconds\n', run, runTime);
    ...
    if ~isempty(validObjectives)
        ...
        if config.output.saveResults
            % Save MAT file
            save(sprintf('results/GA_results_%s.mat', timestamp), ...
                'allResults', 'config', 'G');
            ...
            fprintf('\nResults saved to:\n');
            fprintf('  MAT file: results/GA_results_%s.mat\n', timestamp);
            fprintf('  CSV files: results/GA_summary_%s.csv\n', timestamp);
        end
